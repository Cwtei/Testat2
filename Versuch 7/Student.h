...
public:
     ...
     *
     *  for the ausgabe() function, the parameter 'std::ostream& out'
     *  is basically saving if its std::cout or std::ofstream or any
     *  output commands, so if its std::cout then it will execute
     *  std::cout<<name... which prints the whole thing
     */
    ...
    /**
     * @brief Vergleichsoperator für Sortierungen nach Matrikelnummer (kleiner).
     *
     * @param student Der zu vergleichende Student.
     * @return true, wenn dieser Student eine kleinere Matrikelnummer hat.
     */
    bool operator < (const Student& student) const;

    /**
     * @brief Vergleichsoperator für Sortierungen nach Matrikelnummer (größer).
     *
     * @param student Der zu vergleichende Student.
     * @return true, wenn dieser Student eine größere Matrikelnummer hat.
     */
    bool operator > (const Student& student) const;
