...
int main()
{
    ...
    do  // Anzeige des Men�s
    {
        std::cout << std::endl
				  ...
				  << "(6): Ausgeliehene Medium ausgeben" << std::endl
                  << "(7): Beenden" << std::endl;
...
void ausgelieheneMedienAusgeben(std::vector<Medium*> &medien)
{
	...
	{
		if ((*it)->getStatus()) // if true (some medium is borrowed)
		{
			std::cout << "*************************************************************" << std::endl;
			std::cout << *(*it);
			// the list comprises of pointers, so the iterator refer to the pointers as well
			// dereferencing it once makes it a pointer pointing to a Medium, dereferencing it again
			// make it the Medium itself
			status = true;
		}
	}
	if (status == false)
	{
		std::cout << "Kein Medium ist ausgeliehen" << std::endl;
	}
}
